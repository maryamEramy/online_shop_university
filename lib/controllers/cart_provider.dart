...
class CartProvider with ChangeNotifier {
  ...
  //   getCart();
  // }
    Future<void> addCart(ProductInfo product) async {
    if (_cartBox == null) {
      throw Exception("Cart box not initialized. Call setUserId first.");
    }

    final existingItems = _cartBox!.values.where(
    ...
    if (existingItems.isNotEmpty) {
      ...
      await _cartBox!.add(product.toMap());
    }
    getCart();
  }
...
//     final cartData = _cartBox!.keys.map((key) {
//       final item = _cartBox!.get(key);
//       return ProductInfo.fromMap(item, key: key);
//     }).toList();
//
//     _cart = cartData.reversed.toList();
//     notifyListeners();
